// PRODUCTION GAS URL
const API_URL =
  "https://script.google.com/macros/s/AKfycbz1wf_SgXdrTCq1gjPqlKxZFHlGO9OrK3fMqzWXJ-BXNQGTBceUNIuK3Pj2WzM2lUCZ/exec";
...
function switchPage(pageId) {
  ...
  const wrapper = document.getElementById("attendanceMode");
  if (wrapper) {
    if (attendancePages.includes(pageId)) {
      wrapper.classList.remove("hidden");
    } else if (pageId === "frontPage") {
...
function switchFrontTab(tabId) {
  document
    .querySelectorAll(".front-content")
  ...
  document.querySelectorAll('[id^="btn-"]').forEach((btn) => {
    btn.className =
      "flex-1 text-center py-3 font-bold text-sm rounded-xl transition text-slate-600 hover:text-slate-900";
  });
  document.getElementById("btn-" + tabId).className =
    "flex-1 text-center py-3 font-bold text-sm rounded-xl transition bg-white text-slate-900 shadow-sm";
}

// ================= NAVIGASI FRONT PAGE ↔ ATTENDANCE =================
function goToAttendance() {
  document.getElementById("attendanceMode").classList.remove("hidden");
  document.getElementById("frontPage").classList.add("hidden");
  document.getElementById("userPage").classList.remove("hidden");
}

function goToFrontPage() {
  document.getElementById("attendanceMode").classList.add("hidden");
  document
    .querySelectorAll(".page-section")
...
function openAjkModal() {
  document.getElementById("ajkModal").classList.remove("hidden");
  document.body.style.overflow = "hidden";
}

function closeAjkModal(e) {
  if (e && e.target !== document.getElementById("ajkModal")) return;
  document.getElementById("ajkModal").classList.add("hidden");
  document.body.style.overflow = "";
}

document.addEventListener("keydown", function (e) {
  if (e.key === "Escape") {
    var m = document.getElementById("ajkModal");
...
async function searchParticipant() {
  ...
  if (!keyword)
    return Swal.fire("Oops", "Sila masukkan maklumat carian", "warning");
  ...
  try {
    ...
    if (!data.success) {
      const eventLabel =
        (window.currentSettings &&
          (window.currentSettings.EVENT_NAME ||
            window.currentSettings.EVENT_SHORT_NAME)) ||
        "acara";
      const confirmWishlist = await Swal.fire({
        title: "Harap Maaf, Kuota Penuh!",
        html:
          '<p class="text-sm">Rekod anda tidak dijumpai dan tempat duduk ' +
          escHtml(eventLabel) +
          ' telah penuh.</p><br><p class="text-sm font-bold text-slate-800">Adakah anda ingin memasukkan nama ke dalam Senarai Menunggu (Wishlist)?</p>',
        icon: "warning",
    ...
    window.pendingAttendanceParticipants = {};

    data.data.forEach((participant, idx) => {
      let statusTag = "";
      let actionHTML = "";

      // Semak Status Pengganti
      if (participant.type === "REPLACED") {
        statusTag = `<div class="bg-red-50 text-red-700 text-xs font-bold px-3 py-2 rounded-lg mb-3 inline-block border border-red-200 shadow-sm">Slot ini telah diganti oleh: ${escHtml(participant.relativeName)}</div>`;
        ...
        if (participant.type === "REPLACEMENT") {
          statusTag = `<div class="bg-blue-50 text-blue-700 text-xs font-bold px-3 py-2 rounded-lg mb-3 inline-block border border-blue-200 shadow-sm">Peserta Pengganti kepada: ${escHtml(participant.relativeName)}</div>`;
        }

        // UI Butang Hadir
        const participantKey = "p" + Date.now() + "_" + idx;
        window.pendingAttendanceParticipants[participantKey] = participant;
        let btnHadir = participant.attendanceOpen
          ? `<button type="button" data-attendance-key="${escAttr(participantKey)}" class="attendance-submit-btn w-full bg-green-600 text-white py-3 rounded-xl font-bold shadow-md hover:bg-green-700 transition">SAYA HADIR</button>`
      ...
      container.innerHTML += `
            <div class="bg-white border-2 border-slate-100 rounded-2xl p-5 mb-4 shadow-md mt-4">
              ...
              <div class="grid grid-cols-2 gap-3 text-sm">
                <div class="col-span-2 border-b pb-2"><p class="text-xs text-slate-400 font-bold uppercase">Nama Penuh</p><p class="font-bold text-slate-800 text-base">${escHtml(participant.nama)}</p></div>
                <div><p class="text-xs text-slate-400 font-bold uppercase">No Telefon</p><p class="font-semibold text-slate-700">${escHtml(participant.phone)}</p></div>
                <div><p class="text-xs text-slate-400 font-bold uppercase">PG Code</p><p class="font-semibold text-slate-700">${escHtml(participant.pgcode || "-")}</p></div>
              </div>
              ${actionHTML}
            </div>`;
    });
    container.querySelectorAll(".attendance-submit-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const participant =
          window.pendingAttendanceParticipants[
            btn.getAttribute("data-attendance-key")
          ];
        if (participant) submitAttendance(participant);
      });
    });
...
async function submitAttendance(participant) {
  ...
  try {
    const response = await fetch(API_URL, {
      method: "POST",
      body: JSON.stringify({ action: "attendance", data: participant }),
    });
...
async function submitPublicReplacement() {
  ...
  if (!oldKeyword || !newNama || !newPhone) {
    return Swal.fire(
      "Isi Maklumat",
      "No HP Asal, Nama Baru, dan No HP Baru wajib diisi!",
      "warning",
    );
  }

  Swal.fire({
    title: "Memproses Gantian...",
    allowOutsideClick: false,
    didOpen: () => Swal.showLoading(),
  });
  try {
    const response = await fetch(API_URL, {
      method: "POST",
      body: JSON.stringify({
        action: "submitReplacementPublic",
        data: { oldKeyword, newNama, newPhone, newEmail, newPgcode },
      }),
    });
    const result = await response.json();
    Swal.close();
    ...
    if (result.success) {
      await Swal.fire(
        "Berjaya!",
        "Gantian nama anda telah selamat disimpan dalam sistem. Pengganti boleh menyemak nama mereka di tab semakan.",
        "success",
      );
...
async function submitPublicWishlist() {
  ...
  if (!nama || !phone)
    return Swal.fire(
      "Isi Borang",
      "Nama dan No. Telefon wajib diisi!",
      "warning",
    );

  Swal.fire({
    title: "Merekod Wishlist...",
    allowOutsideClick: false,
    didOpen: () => Swal.showLoading(),
  });
  try {
    const response = await fetch(API_URL, {
      method: "POST",
      body: JSON.stringify({
        action: "submitWishlist",
        data: { nama, phone, email, pgcode },
      }),
    });
    const result = await response.json();
    Swal.close();
    ...
    if (result.success) {
      Swal.fire(
        "Terima Kasih!",
        "Nama anda telah selamat dimasukkan ke dalam Senarai Wishlist. AJK akan menghantar WhatsApp jemputan jika ada kekosongan slot.",
        "success",
      );
      ...
      Swal.fire(
        "Gagal",
        result.message || "Sistem gagal merekod wishlist.",
        "error",
      );
    }
  } catch (e) {
...
async function loginAdmin() {
  ...
  try {
    ...
    Swal.fire(
      "Ralat Server",
      "Sistem gagal berhubung. Sila pastikan URL API dan tetapan Deployment GAS adalah tepat.",
      "error",
    );
...
async function loadDashboard() {
  try {
    ...
    const response = await fetch(API_URL, {
      method: "POST",
      body: JSON.stringify({ action: "getDashboard", token: token }),
    });
    const data = await response.json();
    ...
    if (!data.success) {
      ...
      document.getElementById("hadirCount").innerText = "—";
      document.getElementById("tidakHadirCount").innerText = "—";
      if (dashboardError) {
        dashboardError.textContent =
          data.message ||
          "Dashboard belum boleh dimuatkan. Sila jalankan System Validation.";
    ...
    if (data.settings) {
      ...
      if (data.settings.EVENT_DATE) {
        ...
        if (el) el.value = data.settings.EVENT_DATE;
      }
      const evtTime =
        data.settings.EVENT_START_TIME || data.settings.EVENT_TIME;
      if (evtTime) {
        const el = document.getElementById("eventTime");
        if (el) el.value = evtTime;
      }
      if (data.settings.OPEN_BEFORE_HOURS) {
        const el = document.getElementById("openBeforeHours");
        if (el) el.value = data.settings.OPEN_BEFORE_HOURS;
      }
    }
    ...
    if (data.list && data.list.length > 0) {
      data.list.forEach((item) => {
        ...
        if (item.waStatus === "SUDAH")
          selectBg = "bg-blue-100 text-blue-700 border-blue-200";
        if (item.waStatus === "ERROR")
          selectBg = "bg-red-100 text-red-700 border-red-200";
        ...
        const waBadge = `
            <select class="text-xs font-bold rounded-full px-2 py-1 outline-none border cursor-pointer text-center appearance-none ${selectBg}"
              onchange="confirmChangeWAStatus(this, '${escAttr(item.pgcode)}', '${escAttr(item.phone)}', '${escAttr(item.nama)}', '${escAttr(item.waStatus)}')">
        ...
        let displayNama = `<span class="font-bold text-slate-800">${escHtml(item.nama)}</span>`;
        let displayPhone = `<span class="font-medium text-slate-600">${escHtml(item.phone)}</span>`;
        let displayPgcode = `<span class="font-medium text-slate-600">${escHtml(item.pgcode || "-")}</span>`;
        let actionBtn = "";
        ...
        if (isApproved) {
          ...
          displayNama = `<span class="font-bold text-blue-700">${escHtml(item.replacement.nama)}</span> <span class="bg-blue-50 text-blue-600 border border-blue-200 text-[9px] px-1.5 py-0.5 rounded font-bold ml-1 uppercase">Ganti</span>`;
          displayPhone = `<span class="font-medium text-slate-800">${escHtml(item.replacement.phone)}</span>`;
          displayPgcode = `<span class="font-medium text-slate-800">${escHtml(item.replacement.pgcode || "-")}</span>`;
          actionBtn = `<button onclick="promptReplace('${escAttr(item.pgcode)}', '${escAttr(item.phone)}', '${escAttr(item.nama)}')" class="text-slate-400 hover:text-yellow-600 transition" title="Ganti Baru"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M11.534 7h3.932a.25.25 0 0 1 .192.41l-1.966 2.36a.25.25 0 0 1-.384 0l-1.966-2.36a.25.25 0 0 1 .192-.41zm-11 2h3.932a.25.25 0 0 0 .192-.41L2.692 6.23a.25.25 0 0 0-.384 0L.342 8.59A.25.25 0 0 0 .534 9z"/><path d="M8 3c-1.552 0-2.94.704-3.857 1.818a.5.5 0 1 1-.771-.636A6.002 6.002 0 0 1 13.917 7H12.9A5.002 5.002 0 0 0 8 3zM3.1 9a5.002 5.002 0 0 0 8.757 2.182.5.5 0 1 1 .771.636A6.002 6.002 0 0 1 2.083 9H3.1z"/></svg></button>`;
        } else if (isPending) {
          displayNama = `<span class="font-bold text-slate-800">${escHtml(item.nama)}</span><br/><span class="inline-block bg-amber-100 text-amber-800 text-[10px] font-bold px-2 py-0.5 rounded-md mt-1 animate-pulse">Mohon Ganti: ${escHtml(item.replacement.nama)}</span>`;
          actionBtn = `
              <div class="flex justify-center gap-1.5">
                <button onclick="processApproval('${escAttr(item.pgcode)}', '${escAttr(item.phone)}', 'APPROVE', '${escAttr(item.replacement.nama)}')" class="bg-green-600 text-white font-extrabold px-2 py-1 rounded text-xs hover:bg-green-700">✓</button>
                <button onclick="processApproval('${escAttr(item.pgcode)}', '${escAttr(item.phone)}', 'REJECT', '${escAttr(item.replacement.nama)}')" class="bg-red-600 text-white font-extrabold px-2 py-1 rounded text-xs hover:bg-red-700">✗</button>
              </div>`;
        } else {
        ...
        html += `
            <tr class="hover:bg-slate-50 transition">
              <td class="p-4"><input type="checkbox" class="wa-checkbox w-4 h-4 accent-yellow-600 cursor-pointer" value="${escAttr(activePhone)}" data-nama="${escAttr(activeNama)}" data-pgcode="${escAttr(activePgcode)}"></td>
    ...
    if (data.wishlist && data.wishlist.length > 0) {
      data.wishlist.forEach((w) => {
        ...
        wishHtml += `
              <tr class="hover:bg-slate-50 transition text-slate-700">
                <td class="p-4 text-xs text-slate-400">${escHtml(w.timestamp)}</td>
                <td class="p-4 font-bold text-slate-800">${escHtml(w.nama)}</td>
                <td class="p-4">${escHtml(w.phone)}</td>
                <td class="p-4 text-slate-500">${escHtml(w.email || "-")}</td>
                <td class="p-4 text-xs font-bold">${escHtml(w.pgcode || "-")}</td>
      ...
      document.getElementById("wishlistTable").innerHTML =
        `<tr><td colspan="6" class="p-6 text-center text-slate-400">Tiada peserta dalam wishlist setakat ini.</td></tr>`;
    }
    return data;
  } catch (error) {
    console.error("Ralat Dashboard:", error);
    return { success: false, code: "DASHBOARD_LOAD_FAILED", message: error.message };
  }
}
...
async function confirmChangeWAStatus(selectEl, pgcode, phone, nama, oldStatus) {
  ...
  const confirm = await Swal.fire({
    title: "Tukar Status?",
    text: `Anda pasti nak tukar status WA untuk ${nama} kepada ${newStatus}?`,
    icon: "question",
    showCancelButton: true,
...
async function changeBulkWAStatus() {
  ...
  if (selected.length === 0)
    return Swal.fire(
      "Pilih Peserta",
      "Sila tick sekurang-kurangnya satu peserta.",
      "warning",
    );
  if (!newStatus)
    return Swal.fire(
      "Pilih Status",
      "Sila pilih status (BELUM/SUDAH/ERROR).",
      "warning",
    );

  const confirm = await Swal.fire({
    title: "Tukar Status Pukal?",
    text: `Anda pasti nak tukar status WA untuk ${selected.length} peserta kepada [ ${newStatus} ]?`,
    icon: "question",
    showCancelButton: true,
    confirmButtonColor: "#d4a017",
  ...
  selected.forEach((box) => {
    listToUpdate.push({
      phone: box.value,
      pgcode: box.getAttribute("data-pgcode"),
    });
  });
  ...
  try {
    ...
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        action: "updateWAStatus",
        token: token,
        data: { isBulk: true, newStatus: newStatus, list: listToUpdate },
      }),
    });
    const result = await response.json();
    Swal.close();
...
async function promptReplace(oldPgcode, oldPhone, oldNama) {
  const { value: formValues } = await Swal.fire({
    ...
    preConfirm: () => {
      ...
      if (!nama || !phone) {
        Swal.showValidationMessage("Nama & Telefon wajib diisi!");
        return false;
      }
      ...
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          ...
          data: {
            oldPgcode,
            oldPhone,
            newNama: formValues.nama,
            newPhone: formValues.phone,
            newEmail: formValues.email,
            newPgcode: formValues.pgcode,
          },
        }),
      });
      const data = await res.json();
      if (data.success) {
        Swal.fire("Berjaya", "Peserta diganti!", "success");
        loadDashboard();
      } else {
...
async function sendBulkWA() {
  ...
  if (!groupLink) {
    return Swal.fire(
      "Tetapan Diperlukan",
      "Sila tetapkan WhatsApp Group Link di Settings (tab Comms) terlebih dahulu.",
      "warning",
    );
  }

  const confirm = await Swal.fire({
    title: `Hantar ${selected.length} Jemputan?`,
    text: "WhatsApp akan dibuka bergilir. Klik 'Send' di setiap tetingkap.",
    icon: "info",
    showCancelButton: true,
    confirmButtonText: "Mula Mesej",
  });
  if (!confirm.isConfirmed) return;

  for (let i = 0; i < selected.length; i++) {
    const box = selected[i];
    const rawPhone = box.value;
    const nama = box.getAttribute("data-nama");
    const pgcode = box.getAttribute("data-pgcode");
    ...
    try {
      ...
      await fetch(API_URL, {
        ...
        body: JSON.stringify({
          action: "updateWAStatus",
          token: token,
          data: { pgcode, phone: rawPhone, newStatus: "SUDAH" },
        }),
      });
    } catch (err) {}
    ...
    if (i < selected.length - 1) {
      await Swal.fire({
        title: `Dibuka untuk ${nama}`,
        text: "Sila tekan 'Send' di WhatsApp, kemudian klik Seterusnya.",
        icon: "success",
        confirmButtonText: "Seterusnya ➡️",
        allowOutsideClick: false,
      });
    }
  }
...
async function saveSettings() {
  ...
  const eventTime = normalizeTimeForInput(document.getElementById("eventTime").value);
  const openBeforeHours = document.getElementById("openBeforeHours").value;
  const token = sessionStorage.getItem("admin_token");
  if (!token) {
    Swal.fire("Sesi Tamat", "Sila login semula.", "warning");
    logoutAdmin();
    return;
  }
  ...
  try {
    ...
    if (!result.success)
      return Swal.fire(
        "Error",
        result.message || "Gagal save settings",
        "error",
      );
...
function deleteSettings() {
  Swal.fire({
    title: "Padam Jadual?",
    text: "Peserta tidak boleh check-in sehingga jadual baru diset.",
    icon: "warning",
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
      ...
      document.getElementById("openBeforeHours").value = "5";
      saveSettings();
    }
  });
}
...
function startCountdown(dateStr, timeStr) {
  ...
  countdownInterval = setInterval(() => {
    ...
    if (distance < 0) {
      ...
      if (container) {
        container.innerHTML =
          '<div class="text-center font-extrabold text-red-600 text-sm uppercase p-2">❌ AKSES TAMAT</div>';
      }
...
async function validateSetupSilent() {
  ...
  const checksDiv = document.getElementById("sysValChecks");
  ...
  try {
    ...
    // Render sheets
    if (data.sheets) {
      sheetsDiv.innerHTML = Object.entries(data.sheets)
        .map(
          ([name, ok]) =>
    ...
    if (checksDiv && data.eventValidation) {
      const checks = [
        ["EVENT_ID", data.eventValidation.eventIdValid],
        ["Kehadiran Event ID", data.eventValidation.attendanceEventIdHeader],
        ["Wishlist Event ID", data.eventValidation.wishlistEventIdHeader],
        ["Claim Secret", data.eventValidation.attendanceClaimSecretConfigured],
      ];
      checksDiv.innerHTML = checks
        .map(
          ([label, ok]) =>
            `<div class="flex items-center gap-2 ${ok ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"} rounded-lg p-2"><span class="font-bold">${ok ? "✓" : "✗"}</span><span class="font-mono">${escHtml(label)}</span></div>`,
        )
        .join("");
    } else if (checksDiv) {
      checksDiv.innerHTML = "";
    ...
    if (data.headerFindings && data.headerFindings.found) {
      ...
      const foundHtml = data.headerFindings.found
        .map(
          (f) =>
      ...
      const missingHtml =
        data.headerFindings.missing && data.headerFindings.missing.length > 0
          ? data.headerFindings.missing
              .map(
                (m) =>
    ...
    if (data.errors && data.errors.length > 0)
      msgs = msgs.concat(data.errors.map((e) => "❌ " + e));
    if (data.warnings && data.warnings.length > 0)
      msgs = msgs.concat(data.warnings.map((w) => "⚠️ " + w));
    ...
    return { success: false, code: "VALIDATION_LOAD_FAILED", message: e.message };
  }
}

async function migrateSheetSchema() {
  const token = sessionStorage.getItem("admin_token");
  if (!token) {
    logoutAdmin();
    return;
  ...
  try {
    const response = await fetch(API_URL, {
      method: "POST",
      body: JSON.stringify({ action: "migrateSchema", token: token }),
    });
    const data = await response.json();
    if (data.code === "AUTH_REQUIRED") {
      logoutAdmin();
    ...
    if (!data.success) {
      const message =
        ...
        /unknown action|sistem sementara/i.test(data.message || "")
          ? "Backend belum dikemas kini. Deploy backend hardened terkini dahulu."
          : data.message || "Struktur Sheet gagal disediakan.";
      return Swal.fire("Gagal", message, "error");
    }

    const validation = await validateSetupSilent();
    await loadAllSettings();
    const dashboard = await loadDashboard();
    if (validation && !validation.success) {
      Swal.fire("Struktur Disediakan", "Validation masih melaporkan isu: " + ((validation.errors && validation.errors[0]) || validation.code || "VALIDATION_FAILED"), "warning");
      return;
    }
    if (dashboard && !dashboard.success) {
      Swal.fire("Struktur Disediakan", "Dashboard belum boleh dimuatkan: " + (dashboard.code || dashboard.message || "DASHBOARD_LOAD_FAILED"), "warning");
      return;
    }
    Swal.fire(
      "Berjaya",
      data.message || "Struktur Sheet berjaya disediakan.",
      "success",
    );
  } catch (e) {
    Swal.fire(
...
function addBenefit() {
  ...
  benefitsData.push({
    text,
    icon: document.getElementById("benefit-icon").value || "✅",
    order: benefitsData.length + 1,
  });
...
const APPROVED_FRONTEND_SETTINGS_KEYS = new Set([
  "EVENT_ID", "EVENT_NAME", "EVENT_SHORT_NAME", "EVENT_DESCRIPTION",
  "EVENT_DATE", "EVENT_START_TIME", "EVENT_END_TIME",
  "EVENT_VENUE", "ORGANISER_NAME", "PARTICIPANT_QUOTA",
  "POSTER_URL", "REGISTRATION_URL", "PRIMARY_COLOR", "SECONDARY_COLOR",
  "REGISTRATION_CTA_TEXT", "MARKETING_COPY", "SPEAKER_INFO",
...
function collectAllSettings() {
  ...
  document.querySelectorAll('[id^="' + prefix + '"]').forEach((el) => {
    ...
    if (!APPROVED_FRONTEND_SETTINGS_KEYS.has(key)) return;
    if (boolFields.includes(key)) {
      settings[key] = el.checked ? "true" : "false";
    } else if (el.type === "time") {
      settings[key] = normalizeTimeForInput(el.value);
    } else if (el.type === "date") {
      settings[key] = normalizeDateForInput(el.value);
    } else {
  ...
  if (!settings.EVENT_DATE && document.getElementById("eventDate"))
    settings.EVENT_DATE = normalizeDateForInput(document.getElementById("eventDate").value);
  if (!settings.EVENT_START_TIME && document.getElementById("eventTime"))
    settings.EVENT_START_TIME = normalizeTimeForInput(
      document.getElementById("eventTime").value,
    );
...
function populateAllSettings(settings) {
  ...
  normalizedSettings.EVENT_START_TIME =
    normalizeTimeForInput(settings.EVENT_START_TIME) ||
    normalizeTimeForInput(settings.EVENT_TIME);
  normalizedSettings.EVENT_END_TIME = normalizeTimeForInput(settings.EVENT_END_TIME);
  normalizedSettings.EVENT_DATE = normalizeDateForInput(settings.EVENT_DATE);
  const prefix = "s-";
  const boolFields = [
    "SYSTEM_ENABLED",
    "WISHLIST_ENABLED",
  ...
  Object.entries(normalizedSettings).forEach(([key, value]) => {
    ...
    if (key === "EVENT_DATE") {
      const el = document.getElementById("eventDate");
      if (el) el.value = normalizeDateForInput(value);
    }
    if (key === "EVENT_START_TIME" || key === "EVENT_TIME") {
      const el = document.getElementById("eventTime");
      if (el && value) el.value = normalizeTimeForInput(value);
    }

    const el = document.getElementById(prefix + key);
    if (!el) return;
    if (boolFields.includes(key)) {
      el.checked = normalizeBooleanSetting(value);
    } else if (key === "EVENT_START_TIME" || key === "EVENT_END_TIME") {
      el.value = normalizeTimeForInput(value);
    } else if (key === "EVENT_DATE") {
      el.value = normalizeDateForInput(value);
    } else {
      el.value =
        key === "EVENT_NAME" || key === "EVENT_SHORT_NAME"
          ? fixEventNameTypo(value)
          : value || "";
    }
  });
...
async function loadAllSettings() {
  ...
  try {
    ...
    if (data.success && data.settings) {
      ...
      showSettingsStatus(
        "Failed to load settings: " + (data.message || "unknown"),
        "text-red-600",
      );
      return data;
    }
    ...
    return { success: false, code: "CONFIG_LOAD_FAILED", message: e.message };
  }
}
    ...
    if (result.success) {
      ...
      if (result.rejected && result.rejected.length > 0) {
        msg +=
          ...
          " field(s) rejected: " +
          result.rejected.map((r) => r.key + ": " + (r.reason || "rejected")).join(", ") +
          ")";
        showSettingsStatus(msg, "text-yellow-600");
      } else {
        showSettingsStatus(msg, "text-green-600");
      }
      const persistence = await verifySettingsPersistence(settings);
      if (!persistence.ok) {
        const detail = persistence.keys && persistence.keys.length ? ": " + persistence.keys.join(", ") : "";
        showSettingsStatus((persistence.code || "PERSISTENCE_MISMATCH") + detail, "text-red-600");
        return;
      }
      window.currentSettings = persistence.settings;
      populateAllSettings(persistence.settings);
      applyPublicSettings(persistence.settings);
    } else {
      const rejected = result.rejected && result.rejected.length
        ? " (" + result.rejected.map((r) => r.key + ": " + (r.reason || "rejected")).join(", ") + ")"
        : "";
      showSettingsStatus(
        "Error: " + (result.message || "Save failed") + rejected,
        "text-red-600",
      );
    }
  } catch (e) {
...
function normalizeSettingForCompare(key, value) {
  if (key === "EVENT_START_TIME" || key === "EVENT_END_TIME") return normalizeTimeForInput(value);
  if (key === "EVENT_DATE") return normalizeDateForInput(value);
  if (["SYSTEM_ENABLED", "WISHLIST_ENABLED", "REPLACEMENT_ENABLED"].includes(key)) {
    return normalizeBooleanSetting(value) ? "true" : "false";
  }
  return String(value ?? "");
}

async function verifySettingsPersistence(sentSettings) {
  const response = await fetch(API_URL + "?action=getConfig");
  const data = await response.json();
  if (!data.success || !data.settings) {
    return { ok: false, code: "CONFIG_RELOAD_FAILED", keys: [] };
  }

  const mismatched = [];
  Object.entries(sentSettings).forEach(([key, value]) => {
    if (key === "EVENT_TIME") return;
    const expected = normalizeSettingForCompare(key, value);
    const actual = normalizeSettingForCompare(key, data.settings[key]);
    if (expected !== actual) mismatched.push(key);
  });

  return { ok: mismatched.length === 0, code: "PERSISTENCE_MISMATCH", keys: mismatched, settings: data.settings };
}
...
function applyPublicSettings(s) {
  ...
  window.dynamicEventName = fixEventNameTypo(s.EVENT_NAME || "");
  window.dynamicEventShort = fixEventNameTypo(s.EVENT_SHORT_NAME || "");
  ...
  window.dynamicSchedule = s.SCHEDULE_JSON
    ? safeJsonParse(s.SCHEDULE_JSON)
    : null;
  window.dynamicClosedMsg = s.CLOSED_MESSAGE || "";
  window.dynamicSuccessMsg = s.SUCCESS_MESSAGE || "";

  const eventName = fixEventNameTypo(s.EVENT_NAME || s.EVENT_SHORT_NAME || "");
  const eventShort = fixEventNameTypo(s.EVENT_SHORT_NAME || eventName);

  // 1. Header title
  setText(
    "publicEventTitle",
    eventName || "Seminar Attendance & Management System",
  );
  // 2. Header subtitle
  setText(
    "publicEventSubtitle",
    s.EVENT_DESCRIPTION || "Aplikasi Pengurusan Kehadiran Peserta",
  );
  // 3. Countdown label
  setText(
    "publicCountdownLabel",
    eventShort ? eventShort + " Bermula Dalam:" : "Acara Bermula Dalam:",
  );
  // 4. Meta tags
  if (eventName) {
  ...
  if (s.POSTER_URL) {
    ...
    if (img) {
      ...
      img.alt = eventName || "Event Poster";
    }
  ...
  if (benefits.length > 0) {
    ...
    if (bl)
      bl.innerHTML = benefits
        .map(
          (b) =>
            `<li class="flex items-start gap-2"><span class="shrink-0">${escHtml(b.icon || "✅")}</span><span>${escHtml(b.text || "")}</span></li>`,
        )
        .join("");
  }
  // 9. Speaker info
  ...
  if (hasReg) {
    ...
    if (btn) {
      btn.textContent = s.REGISTRATION_CTA_TEXT;
      btn.href = s.REGISTRATION_URL;
    }
  }
  // 11. Quota
  setText(
    "publicQuotaLabel",
    s.PARTICIPANT_QUOTA
      ? "Kuota Penuh " + s.PARTICIPANT_QUOTA + "/" + s.PARTICIPANT_QUOTA
      : "Kuota Penuh",
  );
  // 12. Branding colors
  ...
  if (s.FOOTER_TEXT) {
    ...
    if (fo)
      fo.innerHTML =
        '&copy; <span id="currentYear"></span> ' + escHtml(s.FOOTER_TEXT);
    ...
    if (fo)
      fo.innerHTML =
        '&copy; <span id="currentYear"></span> ' + escHtml(s.ORGANISER_NAME);
    const yEl = document.getElementById("currentYear");
    if (yEl) yEl.textContent = new Date().getFullYear();
  }
  // 15. Success message
  ...
  if (eventName && s.EVENT_DATE && s.EVENT_VENUE) {
    const dateStr = new Date(s.EVENT_DATE).toLocaleDateString("ms-MY", {
      day: "numeric",
      month: "long",
      year: "numeric",
    });
    setHtml(
      ...
      "<b>Photo Album | " +
        ...
        dateStr +
        " | " +
        escHtml(s.EVENT_VENUE) +
        "</b><br><i>Access PIN dalam WhatsApp Group</i>",
    );
...
function fixEventNameTypo(s) {
  return String(s || "").replace(
    /Berani\s+Buar\s+Video/gi,
    "Berani Buat Video",
  );
}
...
function generatePhysicalPDF() {
  if (!window.lastFetchedList || window.lastFetchedList.length === 0) {
    return Swal.fire(
      "Tiada Data",
      "Sila tunggu sehingga dashboard selesai memuatkan data senarai terlebih dahulu.",
      "warning",
    );
  }
  ...
  const dateStr = settings.EVENT_DATE
    ? new Date(settings.EVENT_DATE).toLocaleDateString("ms-MY", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
  ...
  const pdfTitle =
    window.dynamicPdfTitle ||
    window.dynamicEventName ||
    "Senarai Kehadiran Fizikal";
...
async function processApproval(pgcode, phone, status, pendingNama) {
  ...
  try {
    ...
    if (result.success) {
      Swal.fire(
        "Berjaya",
        `Permohonan gantian telah selesai diproses.`,
        "success",
      );
